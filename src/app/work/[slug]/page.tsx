...
export default async function WorkCaseStudyPage(props: Props) {
	...
	return (
		<article>
			...
			<header className="border-b border-(--border) pt-32 pb-16">
				<div className="mx-auto max-w-7xl px-6 lg:px-8">
					...
					<div className="flex items-center gap-3 mb-6">
						...
						<span className="text-xs font-mono text-(--text-subtle)">· {work.year}</span>
					</div>
					...
					<div className="flex flex-wrap gap-2 mt-8">
						{work.tags.map((tag) => (
							<span
								key={tag}
								className="px-2.5 py-1 text-xs border border-(--border) text-(--text-muted) bg-(--bg-subtle) font-mono"
							>
								{tag}
							</span>
						))}
			...
			{/* Overview */}
			<section className="py-16 border-b border-(--border)" aria-label="Project overview">
				<div className="mx-auto max-w-7xl px-6 lg:px-8">
					<div className="max-w-3xl">
						<p className="text-xs font-mono uppercase tracking-widest text-(--brand) mb-4">Overview</p>
						<p className="text-xl text-(--text-muted) leading-relaxed">{work.description}</p>
					</div>
				</div>
			</section>
			...
			{(work.challenge || work.solution || work.outcome) && (
				<section className="border-b border-(--border)" aria-label="Case study details">
						...
						{work.challenge && (
							<div className="px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-12 gap-8">
								...
								<div className="lg:col-span-9">
									<p className="text-(--text-muted) leading-relaxed">{work.challenge}</p>
								</div>
							</div>
						)}
						{work.solution && (
							<div className="px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-12 gap-8 bg-(--bg-subtle)">
								<div className="lg:col-span-3">
									<p className="text-xs font-mono uppercase tracking-widest text-(--text-subtle)">
										Our Solution
									</p>
								...
								<div className="lg:col-span-9">
									<p className="text-(--text-muted) leading-relaxed">{work.solution}</p>
								</div>
							</div>
						)}
						{work.outcome && (
							<div className="px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-12 gap-8">
								...
								<div className="lg:col-span-9">
									<p className="text-(--text) leading-relaxed font-medium">{work.outcome}</p>
								</div>
							</div>
						)}
					</div>
			...
			{/* CTA */}
			<section className="py-20 bg-(--bg-subtle)" aria-label="Start a project">
				<div className="mx-auto max-w-7xl px-6 lg:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
					...
					<div className="flex flex-wrap gap-3">
						...
						<Link
							href="/work"
							className="inline-flex items-center gap-2 px-6 py-3 border border-(--border) text-(--text) text-sm font-medium hover:bg-(--bg) transition-colors"
						>
							View All Work
						</Link>
					</div>
